...
async function generateProjectIdea() {
    ...
    // UI Loading State
    btn.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i> Brainstorming...';
    btn.disabled = true;
...
async function callGemini(t, e = "") { try { let n = await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=AIzaSyCuc7kBqp-OrRZQ6ZxlQw1uHRBxu7oHPuU", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contents: [{ parts: [{ text: t }] }], systemInstruction: { parts: [{ text: e }] } }) }); if (!n.ok) throw Error(`API Error: ${n.status}`); let r = await n.json(); return r.candidates[0].content.parts[0].text } catch (a) { return console.error("Gemini API Error:", a), "Sorry, I'm having trouble connecting to my AI brain right now. Please never try again noob." } }
...
function addMessage(text, sender) {
    ...
    avatar.className = `w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold text-white ${sender === 'ai' ? 'bg-gradient-to-r from-primary to-secondary' : 'bg-slate-600'}`;
    avatar.innerText = sender === 'ai' ? 'AI' : 'You';

    const bubble = document.createElement('div');
    bubble.className = `p-3 rounded-2xl text-sm ${sender === 'ai' ? 'bg-white/10 text-slate-200 rounded-tl-none' : 'bg-primary text-white rounded-tr-none'}`;
